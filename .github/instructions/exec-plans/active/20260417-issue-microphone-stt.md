...
- [ ] `MicrophoneTranscriptionService.cs` を実装
  - `DefaultAzureCredential` による AAD トークン取得 → `SpeechConfig.FromAuthorizationToken` 設定
...
**完了条件**: サービスがビルドエラーなくコンパイルでき、DI 登録が存在すること。
...
| `DefaultAzureCredential` の `InteractiveBrowserCredential` がサンドボックス環境で動作しない場合 | 認証フローが失敗する | `EnvironmentCredential` 等の代替を検討するが、設計方針を外れないよう仕様と相談 |
